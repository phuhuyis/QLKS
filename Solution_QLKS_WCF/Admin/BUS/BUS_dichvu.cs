...
namespace Admin.BUS
{
    public class BUS_dichvu
    {
        ...
        public void loadbangdv(DataGridView dgv, frmadd frm)
        {
            ...
            column2.Items.Add("Lon");
            column2.Items.Add("Chai");
        ...
        public void them(DataGridView dgv)
        {
            ...
            for (int i = 0; i < dgv.RowCount - 1; i++)
            {
                ...
                if (tendv != null && tendv.Trim().Length > 0)
                {
                    ...
                    {
                        loi = true;
                        MessageBox.Show("cột tên dịch vụ ở dòng " + dgv.Rows[i].Cells[0].Value.ToString() + " quá 100 ký tự", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        break;
                    }
                    else
                    {
                        ...
                        {
                            loi = true;
                            MessageBox.Show("cột tên dịch vụ ở dòng " + dgv.Rows[i].Cells[0].Value.ToString() + " có ký tự đặc biệt", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            break;
                        }    
                        else
                        {
                ...
                {
                    loi = true;
                    MessageBox.Show("chưa nhập cột tên dịch vụ ở dòng " + dgv.Rows[i].Cells[0].Value.ToString(), "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
                }
            }
            if (!loi)
            {
                if (dgv.Rows.Count > 1)
                {
                    ...
                    table.TableName = "dichvu";
                    Program.qlks.themdichvu(table);
                    dgv.Rows.Clear();
                }
                else
                {
                    MessageBox.Show("Chưa nhập dịch vụ nào", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
        ...
        {
            Program.qlks.xoadichvu(ma);
        }

        public void load(String madv, TextBox tennv, NumericUpDown gia, ComboBox dvt, frm_Sua_dich_vu frm)
        {
            DataTable table = Program.qlks.loaddichvuql(madv);
            ...
            else
            {
                ...
                dvt.Text = table.Rows[0][2].ToString();
                gia.Value = decimal.Parse(table.Rows[0][3].ToString());
            }
        }

        public void sua(String madv, TextBox tennv, NumericUpDown gia, ComboBox dvt, frm_Sua_dich_vu frm)
        {
            bool loi = false;
            if (tennv.Text.Trim().Length > 0)
            {
                if (tennv.Text.Trim().Length > 100)
                {
                    ...
                    MessageBox.Show("Tên dịch vụ nhiều hơn 100 ký tự", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    loi = true;
                }
                else
                {
                    ...
                    {
                        MessageBox.Show("Tên dịch vụ có ký tự đặc biệt", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        loi = true;
                    }    
                    else
                    {
            ...
            {
                //tennv rong
                MessageBox.Show("Hãy nhập tên dịch vụ", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                loi = true;
            }
            if (!loi)
            {
